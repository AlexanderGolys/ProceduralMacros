...
mkNode = (open, items, close, delim) -> TokenTree {
    Opening => open, Items => new MutableList from items, Separator => delim, Closing => close
}
...
tokenTree BasicList := TokenTree => node -> (
    if isEOF node then error("tokenTree: a dangling quote keyword consumed EOF (M2 parse bug); source likely ends with one of " | demark(", ", sort values quoteKeyword));
    if isCSTToken node then leaf leafText node
    else if not isNode node then delimited(";", apply(toList node, tokenTree))
    else (
        built := runHooks(symbol tokenTree, node);
        if built === null then error("tokenTree: unhandled CST tag " | node#0);
        built
    )
)
...
tokenClass String := String => s -> (
    ...
    else if match("^[A-Za-z]", s) then (if m2Keywords#?s then "keyword" else "symbol")
    else "punctuation"
)
...
child = method()
child(TokenStream, ZZ) := TokenStream => (ts, i) -> new TokenStream from {append(chainOf ts, (contentOf focus ts)#i)}

up = method()
up TokenStream := TokenStream => ts -> (
    if atTop ts then error "up: already at the root";
...
-- method dispatch enforces that the second argument is a Function
installMacro = method()
installMacro(String, Function) := Function => (name, fn) -> MacroRegistry#name = fn

-- a macro may return either a TokenStream (a cursor) or a bare TokenTree
resultSource = r -> (
...
expandInvocation = (src, i) -> (
    ...
    name := substring(i + 1, j - i - 1, src);
    if not MacroRegistry#?name then error("expandSource: unknown macro $" | name);
    k := j;
    while k < n and not closesMacro(src, k) do k = pastConstruct(src, k);
    if k >= n then error("expandSource: unterminated macro $" | name);
    block := substring(j, k - j, src);
    result := (MacroRegistry#name)(tokenStream tokenTree cstParse block);
    (resultSource result, k + 1)
)
